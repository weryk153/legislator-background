...
function parseCareers(wt: string): Array<{ title: string; start: string; end: string | null }> {
  ...
  for (const m of wt.matchAll(/\|\s*office(\d*)\s*=\s*([^\n]+)/g)) offices.set(m[1], m[2]);
  ...
  for (const off of list) {
    ...
    for (let a = 0; a < 3 && !wt; a++) {
      try {
        ...
        const r = await fetch('https://zh.wikipedia.org/w/api.php?action=parse&prop=wikitext&format=json&redirects=1&page=' + encodeURIComponent(off.name), { headers: { 'user-agent': UA } });
