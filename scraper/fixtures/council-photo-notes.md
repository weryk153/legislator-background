...
**2026-07-24 再補記：其中 10 位改用議會官網彩色照，取代上述公報黑白照**（見
`photo-attributions.md` 開頭新增小節有完整來源網址）。關鍵發現與教訓：

- **「已解職 ≠ 頁面已刪除」**：多數議會 CMS 只是把離任議員從「現任名單」的連結中移除，
  DB record／實體檔案往往還在，直接訪問舊網址（或用同一 GUID／id 命名規則找鄰近記錄）
  常常還能拿到彩色照。判斷順序建議：先試「同一 URL 現在還活著嗎」，再試「歷屆／選區
  篩選器裡有沒有留著舊記錄」，最後才落回公報備援。
- **屏東 ptcc.gov.tw**：現任名單（`Page=Persional`）的「屆次」下拉選單（`ptp_owns=16..20`）
  即使選到已離任者所屬的**上一屆**，該屆全體名單仍完整保留、含 `PersionalDetail&Guid=`
  連結可直接訪問（用此法補到王景山/潘連周/王啟敏/郭再添）。吳亮慶則是使用者已知本屆
  Guid 直接命中。張振亮因僅遞補在職 9 個月、從未参選過其他屆次，遍查 16~20 屆與全文搜尋
  皆無個人頁存在跡象，判斷為真正不可得。
- **宜蘭 ilcc.gov.tw**：現任名單缺號（A2007/A2012/A2030 等）不能直接當作「就是目標人物」
  ——A2012 經 Wayback 名單快照核對後其實是另一位無關的已離任議員（陳俊宇），必須用快照
  比對姓名↔代碼後才能確認，不可只憑缺號位置猜測。
- **臺東 taitungcc.gov.tw**：`/api/member?kind=N` 列表雖然只回傳目前在任者，但
  `/api/member/{id}?desk=front`（單筆 API，瀏覽器開發者工具的 Network 分頁可直接看到
  該路由）**不受名單篩選限制，只要 id 存在就回傳完整資料**（含 `vis:0` 隱藏旗標），
  用當前已用 id 集合（1~35）反推缺號即可找到隱藏記錄；`id=34` 是站方測試用假資料
  （`member:"test"`），使用前要排除。
- **彰化 chcc.gov.tw**：施嘉華個人頁（id=158）本身已 404，但圖檔在 CDN（`df_ufiles/`）
  仍可獨立訪問——先用 Wayback 找到舊個人頁快照確認檔名（`s_3_07施嘉華.jpg`），再直接
  組網址試，不需要頁面本身存在。這是本批唯一「頁面死了、圖還活著」的案例。
- **花蓮 hlcc.gov.tw**：張正治個人頁（`councillor-data.php?index_no=19`，上屆存留）本身
  仍完整可訪問（含歷年書面質詢 PDF 連結），但 `<img src="upfile/" />` 檔名欄位已被清空
  ——頁面「活著」不保證照片欄位也活著，需個別驗證每個欄位而非只看 HTTP 200。
- **雲林 ylcc.gov.tw**、**新竹縣 hcc.gov.tw**：分別因「舊站相片是內嵌 base64、新站
  2026-04 才上線已來不及收錄她」「多份在職窗口內的名單快照與現網比對後從未出現過
  這個人」，結構性確認不可得，維持公報照。
