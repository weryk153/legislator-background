...
- 比對鍵：姓名完全相等＋選舉名稱蘊含的公職類型（立委/議員/縣市長）＋現任＋唯一。
- 整批明細檔無參選人選舉區欄位；同名同職類多於一人 → ambiguous，一律進 review 清單人工確認（可用選區佐證），絕不自動掛。
- 比對不到或模糊 → 不入庫，輸出 review 清單（比照 judgments 流程）。

## 邊界處理

- **不分區立委**：獻金掛政黨名下，無個人專戶。頁面層以 district 判斷（不分區/全國），顯示說明文字；資料不另加標記欄位。
